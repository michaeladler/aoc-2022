...
cave. As you do, you notice a collection of [geodes][1] around the pond. Perhaps you could use the obsidian to create
...
open 9 geodes, so its quality level is `*9*`. The second blueprint has ID 2 and can open 12 geodes, so its quality
level is `*24*`. Finally, if you *add up the quality levels* of all of the blueprints in the list, you get `*33*`.
...
hurry; you figure you probably have *32 minutes* before the wind changes direction again and you'll need to get out of
...
In 32 minutes, the largest number of geodes blueprint 1 (from the example above) can open is `*56*`. One way to
achieve that is:
...
However, blueprint 2 from the example above is still better; using it, the largest number of geodes you could open in
32 minutes is `*62*`.
...
[2]: 19/input
[3]: https://twitter.com/intent/tweet?text=I%27ve+completed+Part+One+of+%22Not+Enough+Minerals%22+%2D+Day+19+%2D+Adven
t+of+Code+2022&url=https%3A%2F%2Fadventofcode%2Ecom%2F2022%2Fday%2F19&related=ericwastl&hashtags=AdventOfCode
[4]: javascript:void(0);
