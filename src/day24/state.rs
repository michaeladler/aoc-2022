...
impl Node {
    ...
    fn left_blizz(&self, p: Point2D<i32>) -> bool {
        (self.left[p.y as usize] & (1 << p.x as usize)) != 0
    }

    fn right_blizz(&self, p: Point2D<i32>) -> bool {
        (self.right[p.y as usize] & (1 << p.x as usize)) != 0
    }

    fn up_blizz(&self, p: Point2D<i32>) -> bool {
        (self.up[p.x as usize] & (1 << p.y as usize)) != 0
    }

    fn down_blizz(&self, p: Point2D<i32>) -> bool {
        (self.down[p.x as usize] & (1 << p.y as usize)) != 0
    }

    /*
    pub fn print(&self, start: Point2D<i32>, end: Point2D<i32>) {
        println!("State:");
        ...
        for y in 1..end.y as usize {
            ...
            for x in 1..width {
                ...
                if count > 1 {
                    ...
                    print!("v");
                } else if self.loc == Point2D::new(x as i32, y as i32) {
                    print!("E");
                } else {
                    print!(".");
                }
